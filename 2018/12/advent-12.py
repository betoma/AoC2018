...
class GrowRule:
    ...
    def fits(self,value,leftwards,rightwards):
        if self.fits_current(value) and self.fits_left(leftwards) and self.fits_right(rightwards):
            return True
        else:
            return False
...
class RowofPots:
    def __init__(self, initial, list_of_rules):
        all_contexts = set(["".join(x) for x in itertools.product(["#","."], repeat=5)])
        self.pots = list()
        self.rules = list()
        ...
        for _ in range(0,no_of_generations):
            ...
            for rule in self.rules:
                if rule.applies_at_rightmost():
                    if rule.fits_left(f"{last_gen[penult_pot].value}{last_gen[last_pot].value}"):

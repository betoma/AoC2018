...
class Opcode:
    def __init__(self,code,current_index,memory,input_stack,rel_base:int):
        ...
        if self.parameter_count > 0:
            self.parameters = self.memory[self.index+1:self.index+self.parameter_count+1]
        else:
    ...
    def run(self):
        ...
        operation = {
            "1": add, "2": multiply, "3": put_in,
            "4": output, "5": jump_if_true, "6": jump_if_false,
            "7": less_than, "8": equals, "9": adj_rel_base,
            "99": halt
        }
...
class Intcode:
    ...
    def run_program(self,input_stack=None):
        while True:
            ...
            code = Opcode(self.memory[self.pointer],self.pointer,self.memory,input_stack,self.base)
